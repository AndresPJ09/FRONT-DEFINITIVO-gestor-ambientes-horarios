  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      if (selectedRow) {
        ...
        await Service.post("/fase/", formData);
      }
      fetchData()
      setIsModalOpen(false)
      setSelectedRow(null)
      showSwal("success", "Fase guardado exitosamente")
    } catch (error) {
      ...
      showSwal("error", "Error al guardar la fase", errorMessage)
    }
  }
    ...
    Swal2.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          showSwal("success", "Fase eliminado correctamente")
          fetchData()
        } catch (error) {
          console.error("Error al eliminar la fase:", error)
          showSwal("error", "Error al eliminar la fase", "Inténtalo de nuevo más tarde.")
        }
      }
    })
  }
