...
export function TableCompetencia() {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      if (selectedRow) {
        await Service.put(`/competencia/${selectedRow.id}/`, formData)
      } else {
        await Service.post("/competencia/", formData)
      }
      fetchData()
      setIsModalOpen(false)
      setSelectedRow(null)
      showSwal("success", "Competencia guardado exitosamente")
    } catch (error) {
      console.error("Error al guardar el competencia:", error)
      ...
      showSwal("error", "Error al guardar el competencia", errorMessage)
    }
  }

  const handleDelete = async (row) => {
    Swal2.fire({
      title: "¿Estás seguro de eliminar este competencia?",
      ...
      if (result.isConfirmed) {
        try {
          await Service.delete(`/competencia/${row.id}/`)
          showSwal("success", "Competencia eliminado correctamente")
          fetchData()
        } catch (error) {
          console.error("Error al eliminar el competencia:", error)
          showSwal("error", "Error al eliminar el competencia", "Inténtalo de nuevo más tarde.")
        }
      }
    })
  }
    ...
    <div className="mt-6 mb-8 space-y-6 bg-gradient-to-br from-blue-gray-50 mt-12 rounded-xl min-h-screen via-white to-white">
      <Card className="bg-gradient-to-br from-blue-gray-50 rounded-xl min-h-screen via-white to-white">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-2xl font-bold">Gestión de Competencias</CardTitle>
          <Button
            variant="filled"
            size="sm"
      ...
      <DynamicModal
        ...
        title={selectedRow ? "Editar competencia" : "Crear Nuevo competencia"}
        fields={modalFields}
...
export default TableCompetencia;
